...
class QueryCategory extends connect {
  ...
  public function getCategoryMenu() {
    $stmt = $this->dbh->prepare("SELECT c.name AS name, c.id AS id, count(*) AS count FROM articles AS a LEFT JOIN categories AS c ON a.category_id=c.id WHERE a.is_delete=0 GROUP BY a.category_id ORDER BY c.id");
    $stmt->execute();
    $return = array();
